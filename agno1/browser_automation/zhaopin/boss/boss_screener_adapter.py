...
- 关键选择器统一维护在 boss_screener_selectors.py
...
from agno1.browser_automation.zhaopin.boss import boss_screener_selectors as S


# ---------------------------------------------------------------------------
# 候选人数据模型（与 zhilian_screener_adapter.py 结构完全一致）
# ---------------------------------------------------------------------------
...
class BossScreenerAdapter:
    ...
    使用方式：
        adapter = BossScreenerAdapter(browser=bm)
...
# 向后兼容别名（旧代码中仍使用 BossResumeAdapter 的地方不需要立即修改）
BossResumeAdapter = BossScreenerAdapter
